...
|---|---|---|---|
| `network_status` | `probes.py:14` — DNS: `getent` ×2 hosts · Internet: ThreadPool `curl 1.1.1.1:80` + `8.8.8.8:80`, fallback `https://1.1.1.1:443`, ICMP informativo · captive hint `probes.py:57` + NM `networking connectivity` divergence `probes.py:check_nm_connectivity` (skipped in netns) | ✅ multi-target, per-probe en `details` + `nm_divergence` | OK exige **≥1 probe TCP**; ICMP solo en `details` (§6) |
...
[
  ...
  {"verb": "network_status", "ipc_action": "network_status", "helper_verb": null, "catalog": "safe", "post_condition": "probes.py multi-target DNS+TCP+https+captive hint + NM connectivity divergence (probes.py:check_nm_connectivity)", "file": "netmedic/netmedic/probes.py", "symbol": "check_nm_connectivity", "file_line": "probes.py:14", "unverified": false},
